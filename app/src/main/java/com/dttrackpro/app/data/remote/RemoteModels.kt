...
data class DeviceGroup(
    ...
    @SerializedName("items") val items: List<RemoteDevice> = emptyList()
)
...
data class RemoteDevice(
    ...
    @SerializedName("sensors") val sensors: List<RemoteSensor> = emptyList(),
...
fun RemoteDevice.toDevice(): Device {
    val ignitionOn = sensors.firstOrNull { it.type == "ignition" }
        ?.value?.equals("On", ignoreCase = true) ?: false
    val batteryPct = sensors.firstOrNull { it.type == "battery" }
